...
ACME `agent-01` server endpoints on the Fastify app are not exposed in alpha.4 (matches `tools/nip-ca-server/` C# parity); the SDK's `nip.acme.AcmeServer` is the canonical reference for ACME and can be embedded in production deployments separately.
...
- **Runtime**: Node.js 22
- **Framework**: Fastify
- **Crypto**: Node.js built-in `crypto` (Ed25519 + AES-256-GCM + PBKDF2) + `@peculiar/x509` (RFC-0002 X.509 builder)
