...
基于 Fastify + better-sqlite3 的 NIP 证书颁发机构实现（NPS-3 §8）。
...
Fastify 上的 ACME `agent-01` server 端点暂未在 alpha.4 暴露（与 `tools/nip-ca-server/` 的 C# 参考保持对齐）；SDK 的 `nip.acme.AcmeServer` 是 ACME 的 canonical 参考实现，可单独嵌入生产部署。
...
- **运行时**：Node.js 22
- **框架**：Fastify
- **加密**：Node.js 内置 `crypto`（Ed25519 + AES-256-GCM + PBKDF2）+ `@peculiar/x509`（RFC-0002 X.509 builder）
- **存储**：better-sqlite3
